...
Handles strategy lifecycle, audit logs, and safety gates via Data Service proxy.
"""
...
class GovernanceManager:
    ...
    def __init__(self, db_manager=None):
        # We now prefer the db_proxy (RemoteWriter) for all operations
        self.writer = RemoteWriter()
        self.db = db_manager # Keep for backward compatibility if needed, but unused mostly
    ...
    def log_strategy_approval(
        ...
        """Log a newly approved strategy via Data Service."""
        ...
        self.writer.execute(sql, params)
    ...
    def get_active_strategy(self) -> Optional[Dict[str, Any]]:
        """Retrieve the currently active strategy via Data Service."""
        sql = """
        ...
        """
        result_df = self.writer.query(sql)

        if not result_df.is_empty():
            row = result_df.to_dicts()[0]
            config = json.loads(row['config_json'])
            config["strategy_hash"] = row['strategy_hash']
            config["stage"] = row['stage']
            config["ttl_expiry"] = row['ttl_expiry']
            return config
        return None

    def promote_strategy(self, strat_hash: str, next_stage: str, approved_by: str):
        """Promote a strategy via Data Service."""
        ...
        sql = """
            ...
            WHERE strategy_hash = ?
        """
        self.writer.execute(sql, [next_stage, approved_by, strat_hash])
    ...
    def log_drift(self, strat_hash: str, metric: str, expected: float, actual: float):
        """Log a drift metric via Data Service."""
        ...
        sql = """
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """
        self.writer.execute(sql, [strat_hash, metric, expected, actual, drift_score, status])
