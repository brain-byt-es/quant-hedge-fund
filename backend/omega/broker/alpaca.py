...
class AlpacaBroker(BaseBroker):
    ...
    async def connect(self) -> bool:
        try:
            ...
            await asyncio.to_thread(self._api.get_account)
            self._connected = True
            logger.info("Connected to Alpaca (Async Proxy)")
            return True
    ...
    async def get_account_info(self) -> Dict[str, Any]:
        ...
        try:
            acct = await asyncio.to_thread(self._api.get_account)
    ...
    async def get_quote(self, symbol: str) -> Dict[str, float]:
        ...
        try:
            quote = await asyncio.to_thread(self._api.get_latest_quote, symbol)
            trade = await asyncio.to_thread(self._api.get_latest_trade, symbol)
            return {
                "bid": float(quote.bp),
                "ask": float(quote.ap),
                "last": float(trade.p),
    ...
    async def get_recent_orders(self, limit: int = 50) -> List[Dict[str, Any]]:
        if not self._connected: return []
        try:
            orders = await asyncio.to_thread(self._api.list_orders, status='all', limit=limit)
            return [{
                "id": o.id,
                "symbol": o.symbol,
                "action": o.side.upper(),
    ...
    async def cancel_all_orders(self) -> int:
        ...
        try:
            await asyncio.to_thread(self._api.cancel_all_orders)
